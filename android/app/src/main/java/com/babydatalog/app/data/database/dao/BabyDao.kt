...
interface BabyDao {
    ...
    // Live rows only: a tombstoned local baby must never adopt a server baby's identity
    @Query("SELECT * FROM babies WHERE lower(trim(name)) = :nameLower AND birthDateMs >= :dayStart AND birthDateMs < :dayEnd AND deletedAtMs IS NULL LIMIT 1")
